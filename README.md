...
```bash
fulfillments-api$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
fulfillments-api$ sam logs -n HelloWorldFunction --stack-name fulfillments-api --tail
```
...
```bash
fulfillments-api$ ruby tests/unit/test_handler.rb
```
